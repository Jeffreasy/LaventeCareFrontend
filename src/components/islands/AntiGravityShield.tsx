...
export function AntiGravityShield() {
        ...
        <div className="text-white/40 font-mono text-sm flex items-center gap-2">
          ...
          >
            <path
              ...
              strokeWidth={2}
              d="M12 4V2m0 20v-2m8-8h2M2 12h2m15.364-6.364l1.414-1.414M4.222 19.778l1.414-1.414m12.728 12.728l1.414 1.414M4.222 4.222l1.414 1.414"
            />
          </svg>
          INITIALIZING GATE...
        </div>
  ...
  const threatColor =
    stats.threat_level === 'defcon-1'
      ? 'text-red-500 bg-red-500/10 border-red-500/30'
      : stats.threat_level === 'defcon-3'
    ...
    <div className="w-full clinical-glass rounded-[var(--radius-feature)] overflow-hidden relative">
      ...
      <div className="relative p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <div
            className={`px-3 py-1 rounded-full border text-xs font-mono uppercase tracking-wider ${threatColor}`}
          >
        ...
        <div className="grid grid-cols-2 gap-4 mb-6 relative">
          <div className="bg-white/5 p-4 rounded-[var(--radius-card)] border border-white/8 flex flex-col justify-center">
            <span className="text-white/60 text-[10px] sm:text-xs font-bold font-ui mb-1 uppercase">
              BLOCKED REQUESTS (24H)
            </span>
            <span className="text-2xl sm:text-3xl font-light text-white font-mono">
              {stats.metrics.total_blocked_requests_24h.toLocaleString()}
            </span>
          </div>
          <div className="bg-white/5 p-4 rounded-[var(--radius-card)] border border-white/8 flex flex-col justify-center">
            <span className="text-white/60 text-[10px] sm:text-xs font-bold font-ui mb-1 uppercase">
            ...
            <span className="text-2xl sm:text-3xl font-light text-white font-mono">
              {stats.metrics.total_bans_24h.toLocaleString()}
            </span>
          </div>
        </div>
        ...
        {stats.active_bans && stats.active_bans.length > 0 && (
          <div className="space-y-2 mt-4 border-t border-white/10 pt-4 relative">
            <span className="text-white/60 text-xs font-bold font-ui mb-2 block uppercase">
            ...
            <div className="max-h-32 overflow-y-auto space-y-2 pr-2 scrollbar-thin scrollbar-thumb-white/10">
              {stats.active_bans.slice(0, 5).map((ban, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between bg-white/3 p-2 rounded border border-white/5 text-xs font-mono"
                >
                  <div className="flex items-center gap-2">
                    {ban.is_recidivist ? (
                      <span className="text-amber-500" title="Recidivist (Repeat Offender)">
                        ⚠️
                      </span>
                    ) : (
                    ...
                    <span
                      className="text-white/70 truncate max-w-[150px] sm:max-w-[200px]"
                      title={ban.reason}
                    >

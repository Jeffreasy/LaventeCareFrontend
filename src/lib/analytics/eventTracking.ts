...
export function trackPricingCTA(
    package_type: 'setup' | 'hosting' | 'iot',
    tier?: string
): void {
    const goalKey =
        package_type === 'setup' ? 'PRICING_SETUP_CTA' :
            package_type === 'hosting' ? 'PRICING_HOSTING_CTA' :
                'PRICING_IOT_CTA';
...
export function trackServiceInteraction(
    ...
    const goalKey = action === 'click'
        ? 'SERVICE_CARD_CLICK'
        : 'SERVICE_PAGE_VIEW';

    trackConversionGoal(CONVERSION_GOALS[goalKey as keyof typeof CONVERSION_GOALS], { service, action });
}

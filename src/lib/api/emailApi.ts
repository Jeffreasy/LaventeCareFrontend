 ...
 */
export async function testMailConnection(config: SMTPConfig): Promise<{ success: boolean; message: string }> {
    // Hash password before sending
    const hashedPassword = await hashPassword(config.password);

    try {
        return await apiClient.post<{ success: boolean; message: string }>('/api/v1/admin/mail-config/test', {
            ...config,
